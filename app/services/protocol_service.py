...
class ProtocolService:
    ...
    def create_protocol(db: Session,
        ...
        """Create a complete validation protocol with duplicate check"""
        ...
        if not previous:
            raise ValueError(f"Previous product with id {previous_product_id} not found")
        
        next_product = db.query(Product).filter(Product.id == next_product_id).first()
        if not next_product:
            raise ValueError(f"Next product with id {next_product_id} not found")
        ...
        # Generate unique protocol number with duplicate check
        protocol_number = ProtocolService.generate_protocol_number()
        existing = db.query(ValidationProtocol).filter(ValidationProtocol.protocol_number == protocol_number).first()
        retry_count = 0
        ...
        protocol = ValidationProtocol(
            ...
            microbiological_acceptance=100,
            ...
            dirty_hold_time_hours=24,
            clean_hold_time_hours=168,
